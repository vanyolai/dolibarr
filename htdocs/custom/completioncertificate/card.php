...
if ($action==='create' && $orderid) {
 ...
 foreach($order->lines as $line){$d=$line->desc ?: $line->label; print '<tr><td>'.dol_htmlentitiesbr($d).'</td><td class="right">'.price($line->qty).'</td><td class="right"><input class="width75 right" name="qty_'.$line->id.'" value="'.price2num($line->qty).'"></td></tr>';}
 print '</table><br><label>'.$langs->trans('PublicNote').'</label><br><textarea class="quatrevingtpercent" rows="4" name="note_public"></textarea><div class="center"><input class="button button-save" type="submit" value="'.$langs->trans('Create').'"></div></form>';
} elseif ($id) {
 ...
 if(!empty($c->note_public)) print '<br><div class="opacitymedium">'.$langs->trans('PublicNote').'</div><div class="wordbreak">'.dol_htmlentitiesbr($c->note_public).'</div>';
}
llxFooter(); $db->close();
