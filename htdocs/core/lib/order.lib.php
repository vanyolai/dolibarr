 ...
 *
 * The function must never break the order card rendering. If optional
 * prefill data cannot be calculated, it falls back to a minimal URL that
 * still links the Agenda event to the source order.
 *
...
function getOrderAgendaCreateUrl(Commande $object, $backtopage = '')
{
	...
	// Build a safe minimal URL first. Everything below is optional prefill data.
	...
	try {
		...
		if (!empty($plannedDate)) {
			...
			foreach ($object->lines as $line) {
				...
				$durationHours *= (float) $line->qty;
				$eventEnd = dol_time_plus_duree($eventEnd, $durationHours, 'h');
				$hasDuration = true;
			}

			if ($hasDuration && $eventEnd > $plannedDate) {
				$url .= '&datef='.dol_print_date($eventEnd, '%Y%m%d%H%M%S', 'tzuserrel');
			}
		}
	} catch (Throwable $e) {
		// A convenience action must not make the whole order card unusable.
		dol_syslog(__FUNCTION__.': Unable to build Agenda prefill: '.$e->getMessage(), LOG_WARNING);
	}
